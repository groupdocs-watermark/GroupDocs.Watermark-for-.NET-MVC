...
namespace GroupDocs.Watermark.MVC.Products.Watermark.Util
{
    public class ColorParser
    {
        ...
        private class NamedCssColors
        {
            ...
            {
                uint namedColor;
                if (namedColors.TryGetValue(input, out namedColor))
                {
                    result = Color.FromArgb((int) namedColor);
                    return true;
                }
        ...
        private static bool TryParse(string input, out Color result, bool throwOnError)
        {
            ...
            if (rgba.Success)
            {
                ...
                if (TryParseRgbComponent(rgba.Groups[2].Value, out red) &&
                    TryParseRgbComponent(rgba.Groups[3].Value, out green) &&
                    TryParseRgbComponent(rgba.Groups[4].Value, out blue) &&
                    TryParseIntegerAlpha(rgba.Groups[5].Value, out alpha))
                {
        ...
        {
            float alpha;
            if (TryParseAlpha(input, out alpha))
            {
        ...
        private static bool TryParseRgbComponent(string input, out int result)
        {
            ...
            float numeric;
            if (!float.TryParse(input, NumberStyles.Float, CultureInfo.InvariantCulture, out numeric))
            {
                result = 0;
                return false;
            }
        ...
        private static bool TryParseHexColor(string input, out Color result)
        {
            if (input.Length < 10)
            {
                ...
                uint numeric;
                if (uint.TryParse(input, NumberStyles.HexNumber, CultureInfo.InvariantCulture, out numeric))
                {
                    switch (input.Length)
                    {
                        // RGB
                        ...
                        // RRGGBB
                        case 6:
                            result = Color.FromArgb((int) ((numeric << 8) | 0xffu));
                            return true;

                        // RRGGBBAA
                        case 8:
                            result = Color.FromArgb((int) numeric);
                            return true;
                    }
                }
            }
